  ...
  return (
    <div className="w-[420px] max-w-full rounded-lg p-7 bg-white">
      ...
      <div className='flex gap-3 mt-4 ml-12'>
        <Button type='primary' onClick={onConfirm} className='flex items-center justify-center w-20 text-center text-white rounded-lg cursor-pointer h-9 '>{confirmText || t('app.chat.confirmBtn')}</Button>
        <Button onClick={onCancel} className='flex items-center justify-center w-20 text-center text-gray-500 border rounded-lg cursor-pointer h-9 border-color-gray-200'>{cancelText || t('app.chat.cancelBtn')}</Button>
      </div>
    </div>
